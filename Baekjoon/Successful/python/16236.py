...
def distance(start):
    q = [(start, 0)]
    sm = [[True]*n for _ in range(n)]
    fish = [[(-1, -1), n*n+1]]
    while len(q) :
        a = q.pop(0)
        if len(fish) > 1 and fish[-1][1] < a[1] : break 
        if 0 < space[a[0][0]][a[0][1]] < babyshark : fish.append(a)
        if not sm[a[0][0]][a[0][1]] or space[a[0][0]][a[0][1]] > babyshark : continue
        else : sm[a[0][0]][a[0][1]] = False
